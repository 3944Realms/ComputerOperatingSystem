...
import top.r3944realms.cos.software.System
import java.io.File
    ...
    while (true) {
        ...
        when (readlnOrNull()?.trim()) {
            "1" -> {
                ...
                runSimulationWithConfig("system.yaml", "default_simulation")
            }
            "2" -> {
               runBatchSimulations()
            }
...
private fun showProcessDetails() {
    val processes = System.getAllProcesses()
    ...
    processes.forEach { process ->
        println("${process.name} (ID: ${process.id.pid}): " +
                "State=${process.state}, " +
...
private fun runBatchSimulations() {
    ...
    configFiles.forEach { configFile ->
        if (File(configFile).exists() ||
            System::class.java.getResource("/$configFile") != null) {
