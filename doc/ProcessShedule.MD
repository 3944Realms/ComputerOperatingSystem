...
1. 启动程序后进入交互式菜单
2. 选择 "1. Run Process Scheduler Demo"
...
2. Run all defined simulations    # 运行所有自设配置
3. Run with custom config         # 使用自定义配置
4. Manual resource operations     # 打开资源操作的二级菜单
5. Show system status             # 显示系统状态
6. Reset system                   # 重置系统
7. Exit                           # 退出程序
```
...
```
1. Request resources      # 请求资源
2. Release resources      # 释放资源
3. Perform safety check   # 执行安全性检查
4. Show current state     # 显示目前系统状态
5. Back to main menu      # 返回主菜单
```

### 选项详解

#### 1. 运行默认模拟
- 使用 `ba_system.yaml` 配置文件
...
- 示例：
  ```
  Enter config file path: ba_custom.yaml
  # 生成日志：logs/ba_custom_simulation.log
  
  Enter config file path: scenarios/ba_custom2.yaml  
  # 生成日志：logs/ba_custom2_simulation.log
  ```
